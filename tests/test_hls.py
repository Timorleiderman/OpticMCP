...
def test_capture_image_returns_base64(mock_cv2):
    """Test HLS capture_image returns base64 string."""
    mock_cap = MagicMock()
    ...
    from optic_mcp.hls import capture_image

    result = capture_image(hls_url="http://example.com/stream.m3u8")
...
def test_capture_image_connection_failed(mock_cv2):
    """Test HLS capture_image raises error when connection fails."""
    mock_cap = MagicMock()
    ...
    with pytest.raises(RuntimeError, match="Could not connect to HLS stream"):
        capture_image(hls_url="http://invalid.com/stream.m3u8")


@patch("optic_mcp.hls.cv2")
def test_capture_image_read_failed(mock_cv2):
    """Test HLS capture_image raises error when frame read fails."""
    mock_cap = MagicMock()
    mock_cap.isOpened.return_value = True
    ...
    with pytest.raises(RuntimeError, match="Failed to capture frame from HLS stream"):
        capture_image(hls_url="http://example.com/stream.m3u8")
...
@patch("optic_mcp.hls.cv2")
def test_save_image_connection_failed(mock_cv2):
    """Test HLS save_image raises error when connection fails."""
    mock_cap = MagicMock()
    mock_cap.isOpened.return_value = False
    mock_cv2.VideoCapture.return_value = mock_cap
    mock_cv2.CAP_FFMPEG = 1900
    ...
    with pytest.raises(RuntimeError, match="Could not connect to HLS stream"):
        save_image(hls_url="http://invalid.com/stream.m3u8", file_path="/tmp/test.jpg")
...
def test_check_stream_unavailable(mock_cv2):
    ...
    result = check_stream(hls_url="http://invalid.com/stream.m3u8")
    assert result["status"] == "unavailable"
    assert "error" in result


@patch("optic_mcp.hls.cv2")
def test_check_stream_read_failed(mock_cv2):
    """Test check_stream returns unavailable when frame read fails."""
    mock_cap = MagicMock()
    mock_cap.isOpened.return_value = True
    mock_cap.read.return_value = (False, None)
    mock_cv2.VideoCapture.return_value = mock_cap
    ...
    result = check_stream(hls_url="http://example.com/stream.m3u8")
    assert result["status"] == "unavailable"
    assert "could not read frame" in result["error"].lower()
